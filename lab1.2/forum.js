...
io.on('connection', function(sock) {
	...
	sock.on('message', function(msgStr){
  	...
  	sock.on('new subject', async function (sbj) {
		var id = await dm.addSubject(sbj);
  	...
  	sock.on ('get user list', async function () {
		console.log("Event: get user list");
		var lista = await dm.getUserList();
  		sock.emit ('user list', lista);
  	});

  	// Client ask for current subject list
  	sock.on ('get subject list',async  function () {
		console.log("Event: get subject list");
		var lista = await dm.getSubjectList()
		sock.emit ('subject list', lista);
  	});

  	// Client ask for message list
  	sock.on ('get message list',async  function (from, to, isPriv) {
		console.log("Event: get message list: " + from + ':' + to + '(' + isPriv + ')');  		
		if (isPriv) {
	  		//sock.emit ('message list', from, to, isPriv, dm.getPrivateMessageList());
	  	} else {
			var lista = await dm.getPublicMessageList(to);
	  		sock.emit ('message list', from, to, isPriv, lista);
	  	}
  	});
